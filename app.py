...
# ---- Helpers ----
def overlaps(start_t, end_t, bookings):
    sdt = datetime.combine(sel_date, start_t)
    edt = datetime.combine(sel_date, end_t)
    ...
    for s,e in slots:
        if st.button(f"Foglal {s.strftime('%H:%M')}-{e.strftime('%H:%M')}"):
            new = {
                ...
                'Időtartam (perc)':dur,
                'Fő':1,'Ismétlődik':False,
                'RepeatGroupID':'','Megjegyzés':''
            }
...
# ---- Admin view ----
    ...
    if menu=="Foglalások":
        weeknum = df['Dátum'].apply(lambda d:d.isocalendar()[1]).max()
        wk = df[df['Dátum'].apply(lambda d:d.isocalendar()[1])==weeknum]
        ...
        for idx,row in wk.iterrows():
            ...
            c1,c2 = st.columns(2)
            if c1.button("Törlés", key=f"del_{idx}"):
                df_drop = df.drop(idx)
                save_df(df_drop, ws)
                st.rerun()
            if c2.button("Csúsztat", key=f"mv_{idx}"):
                nt = st.time_input("Új kezdés", value=datetime.strptime(row['Kezdés'],"%H:%M").time())
                df.at[idx,'Kezdés'] = nt.strftime("%H:%M")
                save_df(df, ws)
                st.rerun()
    elif menu=="Lovak":
        st.write("Lovak kezelés - hamarosan.")
    elif menu=="Statisztika":
        st.write("**Top 10 név:**")
        st.dataframe(df['Gyermek(ek) neve'].value_counts().head(10))
        st.write("**Lovak használat:**")
        horses = df['Lovak'].fillna("").str.split(",").explode().str.strip()
        st.dataframe(horses[horses!=""].value_counts())
    else:
        users_df, users_ws = get_users_df()
        st.write("**Felhasználók:**")
        st.dataframe(users_df)
        st.write("**Új felhasználó:**")
        nu = st.text_input("Felhasználónév")
        npw= st.text_input("Jelszó", type="password")
        if st.button("Hozzáadás"):
            users2 = pd.concat([users_df, pd.DataFrame([{'username':nu,'password':npw}])], ignore_index=True)
            save_users(users2, users_ws)
